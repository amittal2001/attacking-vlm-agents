...
def config() -> argparse.Namespace:
    ...
    # pgd attack related
    parser.add_argument("--N", type=int, default=10, help="Number of steps for random smoothing")
    parser.add_argument("--sigma", type=float, default=0.1, help="Sigma for random smoothing")
...
def test(
    ...
    wandb_run = wandb.init(
        ...
        name=f"RundSmooth-N{args.N}-Sigma{args.sigma}-",         
        config={
    ...
    for domain in tqdm(test_all_meta, desc="Domain"):
        for example_id in tqdm(test_all_meta[domain], desc="Example", leave=False):
            ...
            try:
                ...
                response, actions, logs, computer_update_args = agent.rand_smooth_predict(
                    instruction,
                    obs,
                    N=args.N,
                    sigma=args.sigma,
                ...
                with open(os.path.join(example_result_dir, "traj.jsonl"), "w") as f:
                    f.write(json.dumps({
                        ...
                        "actions": actions,
                        "logs": logs,
                        "computer_update_args": computer_update_args
                    }) + "\n")

...
"""
    ...
    def rand_smooth_predict(self, instruction: str, obs: Dict, N=10, sigma=0.1, wandb_run=None) -> List:
        """
        Predict the next action(s) based on the current observation.
        """
        logs={}
        ...
        if not override_plan:
            ...
            # send to gpt
            logger.info("Thinking...")
            plan_result = self.planner.rand_smooth_plan(last_image, user_question, N=10, sigma=0.1)

            if wandb_run:
                wandb_run.log({"original_image": wandb.Image(last_image)})

                table = wandb.Table(columns=["user_question"])
                table.add_data(user_question)
                wandb_run.log({"user_question_table": table})

                response_table = wandb.Table(columns=["model_response"])
                response_table.add_data(plan_result)
                wandb_run.log({"model_response_table": response_table})

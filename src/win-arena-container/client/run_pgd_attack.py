...
def config() -> argparse.Namespace:
    ...
    # pgd attack related
    parser.add_argument("--epsilon", type=float, default=0.03, help="Epsilon for PGD attack")
...
def test(
    ...
    attacker = VLMWhiteBoxPGDAttack(
        agent=agent,
        target_action_str=args.target_action,
    ...
    for domain in tqdm(test_all_meta, desc="Domain"):
        for example_id in tqdm(test_all_meta[domain], desc="Example", leave=False):
            ...
            try:
                result = attacker.run_attack(env, example, save_dir=example_result_dir)
                ...
                with open(os.path.join(example_result_dir, "traj.jsonl"), "w") as f:
                    ...
                    f.write(json.dumps({
                        "final_agent_output": result["final_action"]
                    }) + "\n")
